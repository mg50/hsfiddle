...
joinAMQP cred = do
  conn <- openConnection (amqpAddr cred) "/" (amqpUser cred) (amqpPass cred)
...
callback ctor pending (msg, envelope) = do
  ...
  case msgReplyTo msg of
    Just msgId -> deliverPending pending (TStrict.pack msgId) $ ctor (Enc.decodeUtf8 bod)
    Nothing    -> return ()
